...
size, image_mode = (128, 128)
# create the original pygame surface
surf = pygame.image.fromstring(size, image_mode)

# create a PIL image and blur it
pil_blured = Image.fromstring("RGBA", size).filter(
    ImageFilter.GaussianBlur(radius=6))

# convert it back to a pygame surface
other = pygame.image.fromstring(
    pil_blured.tostring("raw", image_mode), size, image_mode)
